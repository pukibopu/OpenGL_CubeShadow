...
int main() {
    ...
    //shader
    Shader shader("shaders/vCubeShader.glsl", "shaders/fCubeShader.glsl");
    Shader samplerShader("shaders/vSamplerDepth.glsl", "shaders/fSamplerDepth.glsl", "shaders/gSamplerDepth.glsl");
    //texture
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        samplerShader.use();
        for (int i = 0; i < 6; i++)
            samplerShader.setMat4fv("shadowMatrices[" + std::to_string(i) + "]", shadowTransforms[i]);
        samplerShader.setFloat("far_plane", far_plane);
        samplerShader.setVec3("lightPos", lightPos);
        renderScene(samplerShader);
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
